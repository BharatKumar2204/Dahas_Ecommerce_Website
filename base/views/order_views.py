...
def addOrderItems(request):
    ...
    if orderItems and len(orderItems)==0:
        ...
        for i in orderItems:
            ...
            # (4) Update Stock
            if item.size=='XL':
                product.countInStock_XL-=item.qty
            if item.size=='L':
                product.countInStock_L-=item.qty
            if item.size=='M':
                product.countInStock_M-=item.qty
            if item.size=='S':
                product.countInStock_S-=item.qty   
...
@api_view(['PUT'])
@permission_classes([IsAdminUser])
def updateOrderToDeliveried(request,pk):
    order=Order.objects.get(_id=pk)
